/*
 *  Copyright (c) 2012 Dmitry Suvorov <D.Suvorov90@gmail.com>
 */
...
#include <vmotion/motion_control/v_engines_couple.h>

#include "config/config_port.h"
#include "v_robot_motion_facade_impl.h"
...
VRobotMotionFacadeImpl::MotorInfo::MotorInfo() noexcept :
    uparams(new vunirobot::DCMotorParameters)
{
}

VRobotMotionFacadeImpl::VRobotMotionFacadeImpl() noexcept :
...
int VRobotMotionFacadeImpl::connectToDevices()
{
    ...
    m_state[UNIROBOT] = result ? 0 : 1;
    return result ? connection::CONNECTION_OK : connection::MOTION_CONNECTION_ERROR;
}
...
bool VRobotMotionFacadeImpl::setSpeed(int velocity, int omega)
{
    ...
    m_state[UNIROBOT] = res ? 0 : 1;
    return res && !commandIgnored;
}

int16_t VRobotMotionFacadeImpl::convertToUnirobotOmega( vreal omega, int motorId ) const noexcept
{
    return static_cast<int16_t>((omega * m_info.at(motorId).gearRatio * m_info.at(motorId).numberOfTicksPerRevolutionEncoder) / ( 2 * M_PI ));
}

vreal VRobotMotionFacadeImpl::convertFromUnirobotOmega( int omega, int motorId ) const noexcept
{
    return omega * ( 2 * M_PI ) / (m_info.at(motorId).gearRatio * m_info.at(motorId).numberOfTicksPerRevolutionEncoder);
}

void VRobotMotionFacadeImpl::readDCMotorParameters(const std::string& config, VRobotMotionFacadeImpl::MotorInfo& info) noexcept
{
...
bool VRobotMotionFacadeImpl::rebooted( unirobot_state_t stateX, unirobot_state_t stateY, unirobot_state_t stateA ) noexcept
{
...
void VRobotMotionFacadeImpl::getDeviceState(DevicesState &deviceState)
{
    deviceState = m_state;
}

void VRobotMotionFacadeImpl::resetErrors()
{
    for(DevicesState::iterator it = m_state.begin(); it !=m_state.end(); ++it) {
        it->second = 0;
    }
}

void VRobotMotionFacadeImpl::ignoreOdometryErrors( bool ignore ) noexcept
{
...
void VRobotMotionFacadeImpl::initUnirobot() noexcept
{
